...
public class Table_User implements Initializable {
	...
	/*@FXML
	 ...
	 String full_name = U_FullName.getText(); String user_name =
	  U_UserName.getText(); String password = U_Password.getText(); String address
	  = U_Address.getText(); String email = U_Email.getText(); String phone =
	  U_Phone.getText(); int role = Integer.parseInt(U_Role.getText()); int city =
 Integer.parseInt(U_City.getText()); int manager =
	Integer.parseInt(U_Manager.getText());
	 
	 // String Sql = "INSERT INTO users VALUES()";
 Window owner =	 b_add.getScene().getWindow(); if (U_FullName.getText().isEmpty()) {
	 AlertHelper.showAlert(Alert.AlertType.ERROR, owner, "Form Error!",
	"Please enter your name"); return; } if (U_Email.getText().isEmpty()) {
AlertHelper.showAlert(Alert.AlertType.ERROR, owner, "Form Error!",
 "Please enter your email id"); return; } if (U_Address.getText().isEmpty()) {
	 AlertHelper.showAlert(Alert.AlertType.ERROR, owner, "Form Error!",
	 "Please enter a password"); return; } try { String SQL =
	"INSERT INTO user (Full_Name,User_Name,U_Password,Email,Address,Phone_Number,Role_ID,City_ID,Manager_ID) VALUES(?,?,?,?,?,?,?,?,?)"
	 ; conn = JDBC_Dao.getConnction(); prstmt = (PreparedStatement)
	 conn.prepareStatement(SQL); prstmt.setString(1, full_name);
 prstmt.setString(2, user_name); prstmt.setString(3, password);
	 prstmt.setString(4, email); prstmt.setString(5, address); prstmt.setString(6,
	 phone); prstmt.setInt(7, role); prstmt.setInt(8, city); prstmt.setInt(9,
	 manager); prstmt.executeUpdate(); JOptionPane.showMessageDialog(null,
	 "Insert suse"); } catch (SQLException | HeadlessException ex) {
